...
# ///
"""Quebec City's heritage buildings, from the Repertoire du patrimoine culturel.
...
Communications under CC-BY, and the same pair of resources Montreal reads:

    classes  c6c20af9  621 province-wide, 90 in Quebec City  -- classified by
                                                                the minister
    cites    ba6bed2e  730 province-wide, 10 in Quebec City  -- cited by the
                                                                municipality

100 landmarks, each with a name, a point and a designation date.  See
`camtl/montreal_landmarks.py` for the two resources' schema divergence and for
why they are unioned on `bien_id` rather than concatenated -- a property can
carry both designations.  No Quebec City property does today, and 90 + 10 comes
to exactly 100, but the register is a live document.

**The city's own portal was the obvious place to look and does not have this.**
Ville de Quebec publishes 35 datasets on Donnees Quebec, of which two are
place-shaped: "Lieux publics" (1,409) is dominated by parking lots (415),
retention basins (210) and service pavilions (110), and carries neither the
Chateau Frontenac, the Citadelle, the Plaines d'Abraham nor the basilica;
"Lieux de culte" is 92 churches.  The provincial register is both the runbook's
...
# It matches the *city* of Quebec, not the province -- the column holds
# municipalities, and the province's own name is not one of them.
MUNICIPALITY = "Québec"
...
if __name__ == "__main__":
    emit(
        transform(
            read_classified() + read_cited(), city_code="CAQUE", prefix="que"
        )
    )
