...
"""Extract Montreal's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run camtl/montreal_osm_extract.py

The scheduled `osm-camtl` [[cloud.job]] is the normal path; this is the manual
