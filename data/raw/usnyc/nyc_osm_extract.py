...
"""Extract New York City's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run usnyc/nyc_osm_extract.py

The scheduled `osm-usnyc` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/usnyc_osm_staging.preql); this script is the
