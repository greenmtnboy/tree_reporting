...
"""Extract Washington DC's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run uswas/washingtondc_osm_extract.py

The scheduled `osm-uswas` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/uswas_osm_staging.preql); this script is the
