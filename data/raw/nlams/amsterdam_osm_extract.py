...
"""Extract Amsterdam's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run nlams/amsterdam_osm_extract.py

The scheduled `osm-nlams` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/nlams_osm_staging.preql); this script is the
