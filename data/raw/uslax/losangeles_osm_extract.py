...
"""Extract Los Angeles's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run uslax/losangeles_osm_extract.py

The scheduled `osm-uslax` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/uslax_osm_staging.preql); this script is the
