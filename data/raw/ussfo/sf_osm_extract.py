...
"""Extract San Francisco's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run ussfo/sf_osm_extract.py

The scheduled `osm-ussfo` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/ussfo_osm_staging.preql); this script is the
