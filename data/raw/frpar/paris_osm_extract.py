...
"""Extract Paris's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run frpar/paris_osm_extract.py

The scheduled `osm-frpar` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/frpar_osm_staging.preql); this script is the
