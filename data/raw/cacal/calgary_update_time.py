...
# ///
"""Freshness probe for Calgary's municipal tree inventory.

Reads `rowsUpdatedAt` off the dataset's view metadata -- one small JSON
response against the 581k-row Public Trees dataset -- so a refresh only re-downloads the
...
Calgary's Parquet silently and for ever).  A genuine outage is caught by
...
DATASET = SocrataDataset("data.calgary.ca", "tfs4-3wwa")
...
if __name__ == "__main__":
    emit_freshness("CACAL", fetch_modified_at)
