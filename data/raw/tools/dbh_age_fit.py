...
"""Fit the genus-level age-to-diameter model tree_predictions.preql falls back on.
    ...
    cd data/raw && uv run tools/dbh_age_fit.py --write     # refit from the published rollup
    cd data/raw && uv run tools/dbh_age_fit.py --check     # exit 1 if the model's block is stale
...
RAW = HERE.parent
COEFFICIENTS_CSV = RAW / "dbh_age_coefficients.csv"
MODEL = RAW / "tree_predictions.preql"
