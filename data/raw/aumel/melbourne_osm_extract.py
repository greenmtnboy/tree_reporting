...
"""Extract Melbourne's OpenStreetMap trees into the staged parquet in GCS.
...
bbox, an extra tag) has somewhere to do it.
    ...
    cd data/raw && uv run aumel/melbourne_osm_extract.py

The scheduled `osm-aumel` [[cloud.job]] is the normal path (see
../trilogy.toml and osm_staging/aumel_osm_staging.preql); this script is the
