...
Emits the mtime of usbos_osm_staging.parquet: the staging file only changes
when boston_osm_extract.py is re-run and the result committed, so that is the
...
STAGING_PATH = Path(__file__).parent / "usbos_osm_staging.parquet"
