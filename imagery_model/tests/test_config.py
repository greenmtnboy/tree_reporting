...
from urban_tree_ml.config import ProjectConfig, load_config, load_studio_config
from urban_tree_ml.evaluation import run_evaluation


def test_crown_center_saved_config_json_contract():
    config = load_config(Path(__file__).parents[1] / "configs" / "boston_naip_external.yaml")
    payload = config.model_dump(mode="json")
    policy = dict(crown_scaled_center=True, crown_center_fraction=0.4,
                  crown_center_max_sigma_m=3.5, crown_estimated_scale=0.6)
    payload["targets"].update(policy)
    parsed = ProjectConfig.model_validate(payload)
    restored = ProjectConfig.model_validate_json(parsed.model_dump_json())
    assert restored == parsed
    assert restored.inventory.city == "USBOS"
    for name, value in policy.items():
        assert getattr(restored.targets, name) == value
    # Old artifacts omit these fields; legacy center targets stay unchanged.
    for name in policy:
        del payload["targets"][name]
    assert not ProjectConfig.model_validate(payload).targets.crown_scaled_center


@pytest.mark.parametrize("field,value", [
    ("crown_center_fraction", 0), ("crown_center_fraction", 1.1),
    ("crown_center_max_sigma_m", 0), ("crown_estimated_scale", -0.1),
    ("crown_estimated_scale", 1.1),
])
def test_crown_center_contract_rejects_invalid_known_values(field, value):
    config = load_config(Path(__file__).parents[1] / "configs" / "sf_naip_smoke.yaml")
    payload = config.model_dump(mode="json")
    payload["targets"][field] = value
...
def test_curated_config_requires_acquisition_dates_and_changes_experiment() -> None:
    config_dir = Path(__file__).parents[1] / "configs"
    citywide = load_config(config_dir / "sf_naip_citywide.yaml")
    ...
    assert curated_values["imagery"].pop("require_acquisition_date") is True
    assert citywide_values["imagery"].pop("require_acquisition_date") is False
