...
def test_registration_review_builds_clickable_ui_without_test_labels(tmp_path: Path) -> None:
    ...
    assert "function validationQueueContext()" in html
    assert "queue.city !== pageParameters.get('city')" in html
    assert 'queue.entries[queue.chips[i]]={split:' in html
    assert 'url.searchParams.set("chip", context.queue.chips[index])' in html
    assert "if (!await syncReviews()) { queueNavigating = false; return; }" in html
    assert "Previous chip" in html and "Next chip" in html
    ...
    assert "WASD moves the group" in html
    assert "boxSelect.textContent = 'Box select'" in html
    assert "selectedByScene[scene.scene_id] = new Set(ids)" in html
    assert "Clamp the shared translation" in html
    assert "tree-marker:not(.group-picked)" in html
    assert "queue.split === 'train' ? '/curate-training'" in html
    assert "selectionFor(scene.scene_id).size !== 1" in html
    assert 'c: "aligned", f: "not-tree", q: "uncertain", e: "duplicate"' in html
    assert "event.key.toLowerCase()==='r'" in html
    assert "const position = displayedPosition(markerSample)" in html
    assert "displayedPosition(samplesById[label.dataset.sampleId])" in html
...
def test_validation_chip_enters_existing_registration_review(tmp_path: Path) -> None:
    ...
    assert "openScene(requestedSceneId)" in html
    assert "Back to chip review" in html
    assert "compare.className = 'compare-models fullscreen-only'" in html
    assert "compare.textContent = 'Compare models'" in html
    assert 'compare.style.cssText' not in html
    assert html.index('badges.append(predictionBadge)') < html.index('badges.append(compare)') < html.index('heuristic.className = "heuristic-button"')
    assert "toggleCurationCompare(card, scene, compare)" in html
    assert "localStorage.getItem('tree-review:compare-enabled') !== '0'" in html
    assert 'void toggleCurationCompare(card, scenesById[card.dataset.scene], compareButton)' in html
    assert "encodeURIComponent(scene.validation_chip_id)" in html
    assert "pointer-events:none" in html
    assert "compare.target = '_blank'" not in html
