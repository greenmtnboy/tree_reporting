...
def test_report_keeps_completion_separate_from_tree_labels_and_scores(tmp_path, monkeypatch):
    ...
    radii = []
    def summaries(radius):
        radii.append(radius)
        return [{"chip_id": "r000000_c000001", "matched": 2,
                 "false_positive": 1, "missed": 3, "ground_truth": 5}]
    bundle = SimpleNamespace(
        metrics={"confidence_threshold": .35, "metrics_by_match_radius_m": {"2.0": {}, "4.0": {}}},
        _chip_summaries=summaries,
    )
    catalog = SimpleNamespace(
        summary=lambda: {"runs": [{"city": "ussfo", "run_id": "example",
                                    "cohort": "validation", "metrics": {"split": "validation"}}]},
        bundle=lambda _: bundle,
    )
    result = city_report(SimpleNamespace(directory=tmp_path, city="ussfo", label="SF"), catalog)
    assert radii == [4.0]
    assert result["evaluation"]["radius_m"] == 4.0
    assert result["evaluation"]["threshold"] == .35
    assert result["evaluation"]["f2"] is None  # No frozen masks: never substitute strict F2.
    previous_summary = catalog.summary
    catalog.summary = lambda: {"runs": [{**previous_summary()["runs"][0],
        "goal_metrics": {"4.0": {"detection": {"f2": .75}}}}]}
    result = city_report(SimpleNamespace(directory=tmp_path, city="ussfo", label="SF"), catalog)
    assert result["evaluation"]["f2"] == .75
    assert result["rows"][1]["f2"] is None
