...
async function gallery(){const key=$('sort').value,limit=+$('limit').value,chips=[...state.chips].sort((a,b)=>key==='worst'?a.detection_f1-b.detection_f1||b.error_score-a.error_score:b[key]-a[key]).slice(0,limit);$('gallery').innerHTML='<div class="empty">Rendering chip overlays…</div>';const cards=await Promise.all(chips.map(card));$('gallery').innerHTML=cards.join('');$('gallery').querySelectorAll('[data-chip]').forEach(image=>image.addEventListener('click',()=>openModal(chipCache.get(image.dataset.chip))));bindTooltips($('gallery'))}
