...
def city_report(context, catalog) -> dict:
    ...
    radius = 4.0
    available = bundle and any(float(k) == radius for k in bundle.metrics["metrics_by_match_radius_m"])
    scores = {row["chip_id"]: row for row in bundle._chip_summaries(radius)} if available else {}
    ...
    goal = (latest or {}).get('goal_metrics') or {}
    goal_at_radius = next((v['detection'] for k,v in goal.items() if float(k)==radius), {})
...
<style>
.cards{align-items:stretch}.card{min-width:0;box-shadow:0 6px 22px #0002}.card h2{margin:0 0 16px}.card .big{font-size:26px;line-height:1.25;font-variant-numeric:tabular-nums}.card p{overflow-wrap:anywhere}.score-panel{display:grid;grid-template-columns:1fr auto;gap:8px;padding:16px;margin-bottom:20px;background:#0f1b14;border:1px solid #375340;border-radius:9px}.score-panel .big{grid-column:2;grid-row:1/3;color:#b8e8c5}.score-panel .action{grid-column:1/-1;justify-self:start;margin-top:6px}.action,button{min-height:40px;justify-content:center;cursor:pointer}button:disabled{opacity:.5;cursor:default}progress{height:12px}#publish-status{align-self:center;overflow-wrap:anywhere}@media(max-width:600px){main{padding:14px}.cards{grid-template-columns:1fr}.card{padding:16px}}
</style>
<div class="controls"><button id="publish-all" disabled>Publish all cities</button><span id="publish-status" role="status" aria-live="polite"></span></div>
<div id="cards" class="cards"></div><div class="controls"><select id="city" aria-label="City"><option value="">All cities</option></select><select id="status" aria-label="Curation status"><option value="">All statuses</option><option value="done">Done</option><option value="in-progress">In progress</option><option value="unreviewed">Unreviewed</option><option value="not-in-review">Not in review set</option></select><select id="split" aria-label="Split"><option value="">All splits</option><option>train</option><option>validation</option><option>test</option></select><select id="sort" aria-label="Sort"><option value="f2">Lowest F2 first</option><option value="id">Tile ID</option></select><input id="search" placeholder="Find tile or scene" aria-label="Find tile or scene"><button id="refresh">Refresh</button></div><p id="count" class="muted"></p><p id="error"></p><div class="table"><table><thead><tr><th>City</th><th>Tile / scene</th><th>Type</th><th>Split</th><th>Status</th><th>Reviewed trees</th><th>Goal-aligned F2 @ 4 m</th><th></th></tr></thead><tbody id="rows"></tbody></table></div>
...
async function load(){try{$('error').textContent='';const response=await fetch('/api/coverage');if(!response.ok)throw Error(await response.text());cities=(await response.json()).cities;const selection=$('city').value;$('city').innerHTML='<option value="">All cities</option>'+cities.map(c=>`<option value="${esc(c.city)}">${esc(c.label)}</option>`).join('');$('city').value=selection;$('cards').innerHTML=cities.map(c=>`<article class="card"><h2>${esc(c.label)}</h2>${c.training_progress?`<div class="big">${pct(c.training_progress.fraction)} of training chips curated</div><progress aria-label="${esc(c.label)} training chips curated" value="${c.training_progress.curated}" max="${c.training_progress.total||1}"></progress><p>${c.training_progress.curated} / ${c.training_progress.total} training chips · ${c.training_progress.without_targets} without retained targets</p><a class="action" href="/curate-training?city=${encodeURIComponent(c.city)}">Curate more training chips</a>`:"<p>Training chip catalog unavailable</p>"}<p>${c.done} / ${c.scenes} review scenes done</p><p>${c.tree_reviews.toLocaleString()} tree reviews · ${c.regions} mask regions</p><p class="muted">${Object.entries(c.split_scenes).map(([s,n])=>esc(s)+': '+n).join(' · ')}<br>${Object.entries(c.verdicts).map(([s,n])=>esc(s)+': '+n).join(' · ')}</p><p>${c.published?'Published feedback is current':'Unpublished changes'}</p>${c.evaluation?`<p>Goal-aligned validation F2: ${pct(c.evaluation.f2)} · ${c.evaluation.chips} chips</p><p class="muted">${esc(c.evaluation.run)}<br>${esc(c.evaluation.cohort)} · confidence ${c.evaluation.threshold}${c.evaluation.radius_m?' · '+esc(c.evaluation.radius_m)+' m':''}</p>`:'<p>No saved evaluation</p>'}</article>`).join('');window.studioViewState?.restore();render()}catch(error){$('error').textContent=error.message}}
let publishing=false;
function polishCards(){
  $('publish-all').disabled=publishing||!cities.length;
  document.querySelectorAll('.card').forEach((card,i)=>{
    const c=cities[i],e=c.evaluation;
    const progress=c.training_progress;
    if(progress&&progress.trees_total!=null){
      const chipHeadline=card.querySelector('.big');
      chipHeadline.classList.remove('big');
      const coverage=document.createElement('section');
      coverage.innerHTML=`<div class="big">${pct(progress.tree_fraction)} of training trees in done tiles</div>
        <progress aria-label="${esc(c.label)} training trees in done tiles" value="${progress.trees_in_done_scenes}" max="${progress.trees_total||1}"></progress>
        <p>${progress.trees_in_done_scenes.toLocaleString()} / ${progress.trees_total.toLocaleString()} unique training trees</p>
        <p class="muted">Retained targets in the current training chip catalog. Done or More done counts; overlapping tiles count each tree once. Small crops contribute only their own trees. Validation/test trees are excluded.</p>`;
      card.insertBefore(coverage,chipHeadline);
    }
    const badge=document.createElement('div');badge.className='score-panel';
    const url='/model?'+new URLSearchParams({city:c.city,run:e?.run||'',radius:'4.0',threshold:String(e?.threshold??.35)});
    badge.innerHTML=`<span class="muted">Goal-aligned validation F2 · 4.0 m</span><div class="big">${pct(e?.f2)}</div><span class="muted">${e?`${e.chips} chips · confidence threshold ${e.threshold}`:'No saved 4 m evaluation'}</span>${e?`<a class="action" href="${esc(url)}">Open validation</a>`:''}`;
    card.insertBefore(badge,card.children[1]);
    for(const p of card.querySelectorAll('p'))if(p.textContent.startsWith('Goal-aligned validation F2:'))p.remove();
  });
}
const originalLoad=load;
load=async()=>{
  await originalLoad();polishCards();
  const city=new URLSearchParams(location.search).get('city');
  if(city)$('city').value=city==='all'?'':city;
  if(document.getElementById('studio-city'))$('city').hidden=true;
  document.querySelectorAll('#cards > .card').forEach((card,i)=>{card.hidden=!!$('city').value&&cities[i].city!==$('city').value});
  $('publish-all').textContent=$('city').value?'Publish city feedback':'Publish all cities';render();
};
$('publish-all').addEventListener('click',async()=>{
  if(publishing)return;publishing=true;$('publish-all').disabled=true;
  const results=[];
  try{
    for(const city of cities.filter(c=>!$('city').value||c.city===$('city').value)){
      $('publish-status').textContent=`Publishing ${city.label}…`;
      try{const response=await fetch('/api/finalize?'+new URLSearchParams({city:city.city}),{method:'POST'});const result=await response.json();if(!response.ok)throw Error(result.error||'Publish failed');results.push(`${city.label}: published`)}
      catch(error){results.push(`${city.label}: FAILED — ${error.message}`)}
    }
  }finally{publishing=false;await load();$('publish-status').textContent=results.join(' · ')+(cities.some(c=>!c.published)?' · New or unpublished edits remain.':'');$('publish-all').disabled=!cities.length}
});
