...
class ImageryConfig(StrictModel):
    ...
    # Latest acquisition day for a mosaic; independent of the STAC search range.
    planting_date_cutoff: date | None = None
...
class TargetsConfig(StrictModel):
    ...
    crown_center_fraction: float = Field(default=0.3, gt=0, le=1)
    crown_center_max_sigma_m: float = Field(default=3.0, gt=0)
    crown_estimated_scale: float = Field(default=0.5, ge=0, le=1)
...
class ModelConfig(StrictModel):
    ...
    backbone: Literal["resnet34", "convnext_tiny", "swin_tiny"]
...
class TrainingConfig(StrictModel):
    ...
    crown_loss_weight: float = Field(default=1.0, ge=0)
