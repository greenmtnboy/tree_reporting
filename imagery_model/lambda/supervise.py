...
if __name__ == "__main__":
    ...
    parser.add_argument("--handoff-seconds", type=int, default=0)
    parser.add_argument("command", nargs=argparse.REMAINDER)
    args = parser.parse_args()
    secret = args.key_file.read_text().strip()
    if args.terminate_only:
        # Never touch persistent storage or sync mounts in the independent watchdog.
        for attempt in range(120):
            try:
                ...
                time.sleep(30)
        raise RuntimeError("Watchdog termination failed")
    if args.check_api:
        request = urllib.request.Request(
    ...
    try:
        ...
        os.sync()
        # A collector acknowledges only after downloading the complete run.
        # Bounded grace retains automatic teardown even if the laptop is offline.
        deadline = time.monotonic() + max(0, min(args.handoff_seconds, 1800))
        ack = args.status_file.with_suffix(".downloaded")
