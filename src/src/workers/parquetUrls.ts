...
export const REMOTE_TREES_PARQUET_URL = `${REMOTE_TREES_BASE_URL}/full_tree_info_v${DATA_VERSION}.parquet`
export const REMOTE_SPECIES_PARQUET_URL = `${REMOTE_TREES_BASE_URL}/tree_enrichment_v${DATA_VERSION}.parquet`
...
export function cityTreeParquetUrl(city: string): string | null {
  ...
  return `${REMOTE_TREES_BASE_URL}/${lower}_tree_info_v${DATA_VERSION}.parquet`
}
...
export const REMOTE_LANDMARKS_PARQUET_URL = `${REMOTE_LANDMARKS_BASE_URL}/full_landmark_info_v${DATA_VERSION}.parquet`
...
export function cityLandmarkParquetUrl(city: string): string | null {
  const lower = city.toLowerCase()
  if (!lower.match(/^[a-z]{2}[a-z]{3}$/)) return null
  return `${REMOTE_LANDMARKS_BASE_URL}/${lower}_landmark_info_v${DATA_VERSION}.parquet`
}
